...
parser.add_option("-p", dest="posi", action="store",help="File path to positive DNAs")
parser.add_option("-n", dest="nega", action="store", help="File path to negative DNAs")
...
model_name = 'fullyconnected_3layer'
trained_model_filename = out + '.h5' # saved as
trained_model_filepath = './models/'+model_name+'/'+trained_model_filename
trained_models_weight_dir ='./models/'+ model_name + '/tensorlog/' + out + 'weights.hdf5'
...
dirpath = trained_models_dir + model_name + 'tesorlog/'
...
X, Y = load_data()
X_train, X_test, Y_train, Y_test = train_test_split(
    X, Y, test_size=0.33, random_state=111) # random setting of the test samples
